...
export const App: React.FC = () => {
  ...
  const { connect, isConnected, room, user } = useQuiz();
  ...
  if (mode === 'participant-login') {
    return <ParticipantLogin onSuccess={handleParticipantSuccess} />;
  }

  if (mode === 'admin-panel' && user?.role === 'admin') {
    return <AdminPanel />;
  }

  if (mode === 'participant-panel' && user?.role === 'participant') {
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            Интерактивная система проведения квизов
          </p>
        </div>
